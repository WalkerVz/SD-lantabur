...
<div class="max-w-6xl mx-auto" x-data="mapelPage()">
    ...
    <div class="w-full overflow-x-auto mb-6 scrollbar-hide">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-2 inline-flex gap-1 min-w-max">
            <a href="{{ route('admin.mapel.index') }}" class="px-4 sm:px-6 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all bg-[#47663D] text-white shadow-md">
                Mapel Umum
            </a>
            ...
            <table class="w-full text-left">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-3 sm:px-6 py-4 text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-wider">Mapel</th>
                        <th class="px-3 sm:px-6 py-4 text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-wider w-16 sm:w-24">KKM</th>
                        <th class="px-3 sm:px-6 py-4 text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-wider w-24 sm:w-32 text-center">Status</th>
                    ...
                    @forelse($mapels as $m)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-3 sm:px-6 py-4">
                                <span class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-gray-100 flex items-center justify-center text-xs sm:text-sm font-bold text-gray-600">{{ $m->urutan }}</span>
                            </td>
                            <td class="px-3 sm:px-6 py-4">
                                <span class="font-bold text-gray-800 text-sm sm:text-base leading-tight">{{ $m->nama }}</span>
                            </td>
                            <td class="px-3 sm:px-6 py-4">
                                <span class="px-2 sm:px-3 py-0.5 sm:py-1 bg-amber-50 text-amber-700 rounded-lg font-bold text-xs sm:text-sm">{{ $m->kkm }}</span>
                            </td>
                            <td class="px-3 sm:px-6 py-4 text-center">
                                <span class="px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold {{ $m->is_aktif ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
