...
<div class="max-w-6xl mx-auto" x-data="praktikPage()">
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-1">Atur kategori praktik, section, dan urutan.</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-2 mb-6 flex flex-col">
        <div class="inline-flex gap-1">
            ...
            <a href="{{ route('admin.mapel.praktik') }}" class="px-6 py-2.5 rounded-xl text-sm font-bold transition-all bg-[#47663D] text-white shadow-md">Praktik</a>
        ...
        <div class="border-t border-gray-100 mt-2 pt-2 inline-flex gap-1">
            <button @click="activeSection = 'all'" :class="activeSection === 'all' ? 'bg-gray-200 text-gray-800' : 'text-gray-500 hover:bg-gray-50'" class="px-4 py-1.5 rounded-lg text-xs font-bold transition-all">Semua</button>
            <template x-for="sec in sections" :key="sec">
                <button @click="activeSection = sec" :class="activeSection === sec ? 'bg-gray-200 text-gray-800' : 'text-gray-500 hover:bg-gray-50'" class="px-4 py-1.5 rounded-lg text-xs font-bold transition-all" x-text="sec"></button>
            </template>
        </div>
    </div>
            ...
            <table class="w-full text-left">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider w-32">Section</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($items as $m)
                        <tr class="hover:bg-gray-50/50 transition-colors" x-show="activeSection === 'all' || activeSection === '{{ $m->section }}'">
                            ...
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 bg-blue-50 text-blue-700 rounded-lg font-bold text-sm">{{ $m->section }}</span>
                            </td>
                            <td class="px-6 py-4 font-bold text-gray-800">{{ $m->kategori }}</td>
