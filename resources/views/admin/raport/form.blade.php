...
<div class="max-w-2xl mx-auto">
    ...
    <form action="{{ $item ? route('admin.raport.update', $item->id) : route('admin.raport.store') }}" method="POST" class="bg-white rounded-xl shadow border border-gray-100 p-6">
        ...
        <div class="space-y-4">
            @foreach(\App\Models\RaportNilai::mapelList() as $key => $label)
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ $label }}</label>
                    <input type="number" name="{{ $key }}" value="{{ old($key, $item?->$key) }}" min="0" max="100" step="0.01" placeholder="0-100" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#47663D] focus:border-[#47663D]">
