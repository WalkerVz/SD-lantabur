...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-wrap gap-2 mb-4 border-b border-gray-200">
        <a href="{{ route('admin.sdm.index') }}" class="px-4 py-3 rounded-t-lg {{ request()->routeIs('admin.sdm.*') ? 'bg-[#47663D] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
            <span class="inline-flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg> SDM Sekolah</span>
        </a>
        <a href="{{ route('admin.struktur.index') }}" class="px-4 py-3 rounded-t-lg bg-[#47663D] text-white">
            <span class="inline-flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h10M9 17h1m10 0h1m-1 0h-1m1 0h-5"/></svg> Struktur Organisasi</span>
        </a>
    </div>

    <div class="bg-white rounded-xl shadow border border-gray-100 p-4 mb-4 flex justify-end">
        <div class="flex gap-2">
            <a href="{{ route('admin.struktur.export.excel') }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-medium">Export Excel</a>
            <a href="{{ route('admin.struktur.create') }}" class="px-4 py-2 bg-[#47663D] text-white rounded-lg hover:bg-[#5a7d52] text-sm font-medium">+ Tambah Struktur</a>
        </div>
    </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            <a href="{{ route('admin.struktur.edit', $r->id) }}" class="inline-block px-3 py-1.5 bg-blue-500 text-white rounded text-sm hover:bg-blue-600">Edit</a>
                            <form action="{{ route('admin.struktur.destroy', $r->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin hapus?');">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-gray-500">Belum ada data struktur.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
