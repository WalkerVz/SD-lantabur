...
<div class="max-w-7xl mx-auto" x-data="newsPage()">
            ...
            <table class="w-full text-left">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            <button type="button" @click="openFormModal({{ $b->id }})" class="inline-block px-3 py-1.5 bg-blue-500 text-white rounded text-sm hover:bg-blue-600">Edit</button>
                            <button type="button" @click="confirmDelete({{ $b->id }}, '{{ addslashes(Str::limit($b->judul, 40)) }}')" class="inline-block px-3 py-1.5 bg-red-500 text-white rounded text-sm hover:bg-red-600">Hapus</button>
                        </td>
                    </tr>
                    @empty
    ...
    <div x-show="formModalOpen" x-cloak class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
        <div class="flex min-h-screen items-center justify-center p-4">
            ...
            <div x-show="formModalOpen" x-transition class="relative bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col">
                <div class="p-4 border-b border-gray-200 flex justify-between items-center">
                    <h3 class="text-lg font-bold text-gray-900" x-text="formModalEditId ? 'Edit Berita' : 'Tambah Berita'"></h3>
                ...
                <div class="flex-1 overflow-y-auto p-4">
                    <iframe :src="formModalUrl" class="w-full border-0 rounded-lg" style="min-height: 520px;" id="news-form-iframe" @load="formIframeLoaded()"></iframe>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="flex min-h-screen items-center justify-center p-4">
            ...
            <div x-show="deleteConfirmOpen" x-transition class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
                <div class="text-center">
                    ...
                    <p class="text-gray-600 text-sm mb-2">Anda yakin ingin menghapus berita <strong x-text="deleteConfirmNama"></strong>?</p>
...
<script>
function newsPage() {
    const createUrl = @json(route('admin.news.create'));
    const editUrl = (id) => '{{ url('admin/news') }}/' + id + '/edit';
    const deleteUrl = (id) => '{{ url('admin/news') }}/' + id;
    return {
        formModalOpen: false,
        formModalUrl: '',
        ...
        closeFormModal() {
            ...
            this.formModalUrl = '';
            if (window._newsMessageHandler) { window.removeEventListener('message', window._newsMessageHandler); window._newsMessageHandler = null; }
        },
        formIframeLoaded() {
        ...
        confirmDelete(id, nama) {
            ...
            this.deleteConfirmNama = nama || 'ini';
