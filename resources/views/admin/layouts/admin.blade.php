...
<body class="bg-gray-50 min-h-screen" x-data="{
    ...
    halamanDepanOpen: {{ request()->routeIs('admin.news.*', 'admin.gallery.*', 'admin.slider.*', 'admin.video.*') ? 'true' : 'false' }},
    ...
    <div class="flex min-h-screen">
        ...
        <aside :class="[
            ...
            <nav class="p-4 space-y-1 flex-1 overflow-y-auto">
                ...
                @if(\App\Models\FeatureAccess::can($currentRole, 'dashboard.view'))
                ...
                @if(\App\Models\FeatureAccess::can($currentRole, 'sdm.view'))
                ...
                @if(\App\Models\FeatureAccess::can($currentRole, 'siswa.view'))
                ...
                @if(\App\Models\FeatureAccess::can($currentRole, 'pembayaran.view'))
                ...
                @if(\App\Models\FeatureAccess::can($currentRole, 'raport.view'))
                ...
                @if(\App\Models\FeatureAccess::can($currentRole, 'mapel.view'))
                <a href="{{ route('admin.mapel.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs('admin.mapel.*') ? 'bg-[#47663D] text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                    <span x-show="!sidebarCollapsed" x-transition>Mata Pelajaran</span>
                </a>
                
                <a href="{{ route('admin.master-kelas.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs('admin.master-kelas.*') ? 'bg-[#47663D] text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                    <span x-show="!sidebarCollapsed" x-transition>Master Kelas</span>
                </a>
                @endif

                {{-- Halaman Depan (dropdown) --}}
                @if(\App\Models\FeatureAccess::can($currentRole, 'halaman_depan.view'))
                ...
                @if(\App\Models\FeatureAccess::can($currentRole, 'settings.view'))
                <div x-data="{ open: settingsOpen }" @click.away="open = false" class="relative">
                    <button @click="open = !open" type="button" class="w-full flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs('admin.settings.index', 'admin.settings.accounts', 'admin.settings.contact') ? 'bg-[#47663D] text-white' : 'text-gray-700 hover:bg-gray-100' }}">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition class="flex-1 text-left">Pengaturan</span>
                        <svg x-show="!sidebarCollapsed" :class="open ? 'rotate-180' : ''" class="w-4 h-4 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                    </button>
                    <div x-show="open" x-transition class="mt-1 pl-4 space-y-0.5 border-l-2 border-gray-200 ml-2">
                        <a href="{{ route('admin.settings.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg {{ request()->routeIs('admin.settings.index') ? 'bg-[#47663D]/10 text-[#47663D]' : 'text-gray-700 hover:bg-gray-100' }}">
                            <span x-show="!sidebarCollapsed">Umum</span>
                        </a>
                        <a href="{{ route('admin.settings.accounts') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg {{ request()->routeIs('admin.settings.accounts') ? 'bg-[#47663D]/10 text-[#47663D]' : 'text-gray-700 hover:bg-gray-100' }}">
                            <span x-show="!sidebarCollapsed">Akun</span>
                        </a>
                        <a href="{{ route('admin.settings.contact') }}" class="flex items-center gap-2 px-3 py-2 rounded-lg {{ request()->routeIs('admin.settings.contact') ? 'bg-[#47663D]/10 text-[#47663D]' : 'text-gray-700 hover:bg-gray-100' }}">
                            <span x-show="!sidebarCollapsed">Kontak</span>
                        </a>
                    </div>
                </div>
